# 🏗️ ThaparMarket Architecture

## System Architecture Diagram

```
┌─────────────────────────────────────────────────────────────────┐
│                         USER DEVICES                             │
│                  (Web Browser - Desktop/Mobile)                  │
└────────────────────────────┬────────────────────────────────────┘
                             │
                             │ HTTPS
                             │
┌────────────────────────────▼────────────────────────────────────┐
│                      FRONTEND (Vercel)                           │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │              Next.js 14 Application                       │  │
│  │                                                           │  │
│  │  • React Components (Shadcn/UI)                          │  │
│  │  • Tailwind CSS Styling                                  │  │
│  │  • React Query (Data Fetching)                           │  │
│  │  • Zustand (State Management)                            │  │
│  │  • Socket.io Client (Real-time)                          │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
└────────────────────┬───────────────────┬────────────────────────┘
                     │                   │
                     │ REST API          │ WebSocket
                     │                   │
┌────────────────────▼───────────────────▼────────────────────────┐
│                    BACKEND (Render)                              │
│                                                                  │
│  ┌──────────────────────────────────────────────────────────┐  │
│  │           Node.js + Express + TypeScript                  │  │
│  │                                                           │  │
│  │  ┌─────────────┐  ┌──────────────┐  ┌────────────────┐  │  │
│  │  │   Routes    │  │ Controllers  │  │   Services     │  │  │
│  │  │             │  │              │  │                │  │  │
│  │  │ • Auth      │→ │ • Auth       │→ │ • JWT          │  │  │
│  │  │ • Listings  │  │ • Listings   │  │ • Email        │  │  │
│  │  │ • Messages  │  │ • Messages   │  │ • Cloudinary   │  │  │
│  │  │ • Users     │  │ • Users      │  │                │  │  │
│  │  │ • Ratings   │  │ • Ratings    │  │                │  │  │
│  │  │ • Admin     │  │ • Admin      │  │                │  │  │
│  │  └─────────────┘  └──────────────┘  └────────────────┘  │  │
│  │                                                           │  │
│  │  ┌─────────────────────────────────────────────────────┐ │  │
│  │  │              Middleware Layer                        │ │  │
│  │  │                                                      │ │  │
│  │  │  • Authentication (JWT)                             │ │  │
│  │  │  • Validation (Joi)                                 │ │  │
│  │  │  • File Upload (Multer)                             │ │  │
│  │  │  • Error Handling                                   │ │  │
│  │  │  • Rate Limiting                                    │ │  │
│  │  │  • Security (Helmet, CORS)                          │ │  │
│  │  └─────────────────────────────────────────────────────┘ │  │
│  │                                                           │  │
│  │  ┌─────────────────────────────────────────────────────┐ │  │
│  │  │              Socket.IO Server                        │ │  │
│  │  │                                                      │ │  │
│  │  │  • Real-time Chat                                   │ │  │
│  │  │  • Typing Indicators                                │ │  │
│  │  │  • Online Status                                    │ │  │
│  │  │  • Notifications                                    │ │  │
│  │  └─────────────────────────────────────────────────────┘ │  │
│  └──────────────────────────────────────────────────────────┘  │
│                                                                  │
└──────┬─────────────────┬─────────────────┬──────────────────────┘
       │                 │                 │
       │                 │                 │
       ▼                 ▼                 ▼
┌──────────────┐  ┌─────────────┐  ┌─────────────────┐
│   DATABASE   │  │  CLOUDINARY │  │   EMAIL (SMTP)  │
│              │  │             │  │                 │
│ Neon         │  │ Image CDN   │  │ Gmail           │
│ PostgreSQL   │  │             │  │                 │
│              │  │ • Upload    │  │ • Verification  │
│ • Users      │  │ • Optimize  │  │ • Welcome       │
│ • Listings   │  │ • Transform │  │ • Password Reset│
│ • Messages   │  │ • Delete    │  │ • Notifications │
│ • Ratings    │  │             │  │                 │
│ • Categories │  │ 25GB Free   │  │ Free            │
│ • Reports    │  │             │  │                 │
│              │  │             │  │                 │
│ 3GB Free     │  │             │  │                 │
└──────────────┘  └─────────────┘  └─────────────────┘
```

## Data Flow Examples

### 1. User Registration Flow

```
User → Frontend → Backend → Database
                    ↓
                  Email Service → User's Email
                    ↓
User enters OTP → Backend → Database (verify)
                    ↓
                  JWT Token → Frontend → User logged in
```

### 2. Create Listing Flow

```
User uploads images → Frontend
                        ↓
                    Backend (Multer)
                        ↓
                    Cloudinary (store images)
                        ↓
                    Get image URLs
                        ↓
                    Database (save listing with URLs)
                        ↓
                    Return success → Frontend
```

### 3. Real-time Chat Flow

```
User A sends message → Frontend (Socket.io client)
                         ↓
                      Backend (Socket.io server)
                         ↓
                      Database (save message)
                         ↓
                      Emit to User B's socket
                         ↓
                      User B receives message (real-time)
```

### 4. Search & Filter Flow

```
User enters search → Frontend
                       ↓
                    Backend API
                       ↓
                    PostgreSQL Full-text Search
                       ↓
                    Apply filters (category, price, etc.)
                       ↓
                    Return paginated results
                       ↓
                    Frontend displays listings
```

## Technology Stack Details

### Frontend Stack
```
Next.js 14 (App Router)
├── React 18
├── TypeScript
├── Tailwind CSS
├── Shadcn/UI Components
├── React Query (Data fetching)
├── Zustand (State management)
├── Socket.io-client (Real-time)
├── Axios (HTTP client)
└── React Hook Form (Forms)
```

### Backend Stack
```
Node.js + Express
├── TypeScript
├── PostgreSQL (Neon)
├── Socket.IO (Real-time)
├── JWT (Authentication)
├── Bcrypt (Password hashing)
├── Multer (File uploads)
├── Cloudinary SDK (Image management)
├── Nodemailer (Email)
├── Joi (Validation)
├── Helmet (Security)
└── Winston (Logging)
```

### Database Schema
```
PostgreSQL (Neon)
├── users (Authentication & Profiles)
├── categories (Marketplace categories)
├── listings (Products/Items)
├── messages (Chat messages)
├── ratings (User reviews)
├── favorites (Saved items)
├── reports (Content moderation)
└── notifications (User alerts)
```

## Security Layers

```
┌─────────────────────────────────────────┐
│  Layer 1: Network Security              │
│  • HTTPS/TLS encryption                 │
│  • CORS configuration                   │
│  • Rate limiting (100 req/15min)        │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│  Layer 2: Application Security          │
│  • Helmet.js security headers           │
│  • Input validation (Joi)               │
│  • SQL injection protection             │
│  • XSS protection                       │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│  Layer 3: Authentication                │
│  • JWT tokens (7-day expiry)            │
│  • Bcrypt password hashing              │
│  • Email verification (OTP)             │
│  • Campus email validation              │
└─────────────────────────────────────────┘
                  ↓
┌─────────────────────────────────────────┐
│  Layer 4: Authorization                 │
│  • Role-based access (User/Admin)       │
│  • Resource ownership validation        │
│  • Admin-only routes protection         │
└─────────────────────────────────────────┘
```

## Deployment Architecture

```
┌──────────────────────────────────────────────────┐
│              Production Environment               │
│                                                   │
│  ┌─────────────┐         ┌──────────────┐       │
│  │   Vercel    │         │    Render    │       │
│  │  (Frontend) │◄───────►│  (Backend)   │       │
│  │             │         │              │       │
│  │  • Next.js  │         │  • Express   │       │
│  │  • CDN      │         │  • Socket.IO │       │
│  │  • SSL      │         │  • SSL       │       │
│  └─────────────┘         └──────┬───────┘       │
│                                 │                │
│                    ┌────────────┼────────────┐   │
│                    │            │            │   │
│              ┌─────▼────┐  ┌───▼────┐  ┌───▼────┐
│              │   Neon   │  │Cloudinary│ │ Gmail │
│              │PostgreSQL│  │   CDN    │  │ SMTP │
│              └──────────┘  └──────────┘  └───────┘
│                                                   │
└──────────────────────────────────────────────────┘

All services on FREE tier! 🎉
```

## Performance Optimizations

### Frontend
- ✅ React Query caching (5-10 min)
- ✅ Image lazy loading
- ✅ Code splitting (Next.js automatic)
- ✅ Static generation where possible
- ✅ Optimized images (WebP, AVIF)

### Backend
- ✅ Database connection pooling
- ✅ Indexed database queries
- ✅ Pagination (limit results)
- ✅ Cloudinary image optimization
- ✅ Gzip compression

### Database
- ✅ Indexes on frequently queried columns
- ✅ Full-text search indexes
- ✅ Foreign key constraints
- ✅ Efficient query design

---

**This architecture supports:**
- 📈 Thousands of concurrent users
- 💬 Real-time messaging
- 🖼️ Efficient image delivery
- 🔒 Enterprise-grade security
- 💰 $0/month cost on free tiers
